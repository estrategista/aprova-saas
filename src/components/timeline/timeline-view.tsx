    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-navy-700 bg-navy-900 p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-sm text-neutral-400">Progresso Geral</p>
            <p className="text-2xl font-bold text-white">{totalTasks.done}/{totalTasks.total} tarefas</p>
          </div>
          {diasRestantes !== null && (
            <div className="text-right">
              <p className="text-sm text-neutral-400">Prazo</p>
        ...
        <p className="text-xs text-neutral-500 mt-1 text-right">{progressPercent.toFixed(0)}% concluido</p>
      </div>
      ...
      {timeline.length === 0 ? (
        <div className="rounded-xl border border-dashed border-navy-600 p-8 text-center">
          ...
          <p className="text-sm text-neutral-400">Nenhuma timeline configurada</p>
        </div>
      ) : (
                ...
                <div className="px-4 py-3 flex items-center justify-between border-b border-navy-800">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {day.horas && <span className="text-xs text-neutral-500">{day.horas}h estimadas</span>}
                    </div>
                  </div>
                  <span className="text-xs text-neutral-400">{doneTasks}/{tasks.length}</span>
                </div>
                {tasks.length > 0 && (
                  <div className="divide-y divide-navy-800">
                    {tasks.map((task: TaskItem, idx: number) => (
                      <button key={idx} onClick={() => toggleTask(day.id, idx)} className="w-full flex items-start gap-3 px-4 py-2.5 text-left hover:bg-navy-800/50 transition-colors cursor-pointer">
                        {task.done ? <CheckCircle2 className="w-4 h-4 text-green-400 mt-0.5 shrink-0" /> : <Circle className="w-4 h-4 text-neutral-500 mt-0.5 shrink-0" />}
                        <div>
                          <p className={`text-sm ${task.done ? "text-neutral-500 line-through" : "text-white"}`}>{task.t}</p>
                          {task.d && <p className="text-xs text-neutral-500 mt-0.5">{task.d}</p>}
                        </div>
                      </button>
                    ))}
                  </div>
